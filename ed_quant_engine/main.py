...
class QuantOrchestrator:
    ...
    def recover_state(self):
        ...
        closed_pnl_val = self.db.fetch_all("SELECT SUM(pnl) FROM trades WHERE status = 'Closed'")
        closed_pnl = closed_pnl_val[0][0] if closed_pnl_val and closed_pnl_val[0][0] else 0
    ...
    async def run_live_cycle(self):
        ...
        for category, tickers in UNIVERSE.items():
            for ticker in tickers:
                ...
                atrs[ticker] = float(df['ATR'].iloc[-1])
        ...
        gc.collect()
        logger.info("Tarama döngüsü tamamlandı, RAM temizlendi.")

async def scheduler_loop(orchestrator):
    while True:
        try:
            now = datetime.now()
            # Phase 23: Sync with top of the hour (minute 00)
            if now.minute == 0:
                await orchestrator.run_live_cycle()
                # sleep for 60 seconds to prevent running multiple times in the same minute
                await asyncio.sleep(60)
            else:
                # Wake up every 30 seconds to check if it's the top of the hour
