...
class DataEngine:
    ...
    def fetch_mtf_data(self, ticker: str) -> pd.DataFrame:
        ...
        htf['EMA_200'] = ta.ema(htf['Close'], length=200)
        macd = ta.macd(htf['Close'])
        htf['MACD'] = macd.iloc[:, 0] if macd is not None else 0
        htf['MACD_Hist'] = macd.iloc[:, 1] if macd is not None else 0

        # Trend check using EMA 50 & 200, MACD
        htf['HTF_Trend'] = np.where((htf['Close'] > htf['EMA_50']) & (htf['EMA_50'] > htf['EMA_200']) & (htf['MACD'] > 0), 1,
                           np.where((htf['Close'] < htf['EMA_50']) & (htf['EMA_50'] < htf['EMA_200']) & (htf['MACD'] < 0), -1, 0))

        # Phase 3 & 16: Anti-Lookahead Bias (Shift HTF by 1)
        htf_shifted = htf[['HTF_Trend', 'EMA_50', 'EMA_200', 'MACD']].shift(1).dropna()

        # Phase 3: Technical Indicators (LTF)
        ltf['RSI'] = ta.rsi(ltf['Close'], length=14)
        ltf['ATR'] = ta.atr(ltf['High'], ltf['Low'], ltf['Close'], length=14)

        bbands = ta.bbands(ltf['Close'], length=20, std=2)
        if bbands is not None:
            ltf['BB_Lower'] = bbands.iloc[:, 0]
            ltf['BB_Upper'] = bbands.iloc[:, 2]
        else:
            ltf['BB_Lower'] = ltf['Close']
            ltf['BB_Upper'] = ltf['Close']

        ltf['Returns'] = ltf['Close'].pct_change()
        # Log Returns
        ltf['Log_Returns'] = np.log(ltf['Close'] / ltf['Close'].shift(1))
